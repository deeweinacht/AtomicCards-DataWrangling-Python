...
def build_tables(dfs: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
    ...
    analytics["cards"] = build_cards_core(dfs["cards"], analytics["card_faces"], dfs["types"], dfs["keywords"], dfs["sets"])
...
def build_faces_core(staging_df: pd.DataFrame) -> pd.DataFrame:
    ...
    core_df["power_num"] = pd.to_numeric(core_df["power"], errors="coerce").astype("Float64")
    core_df["toughness_num"] = pd.to_numeric(core_df["toughness"], errors="coerce").astype("Float64")
    core_df["loyalty_num"] = pd.to_numeric(core_df["loyalty"], errors="coerce").astype("Float64")
    core_df["defense_num"] = pd.to_numeric(core_df["defense"], errors="coerce").astype("Float64") # never variable, so the variable flag is not added for this one
    core_df["power_is_variable"] = (core_df["power"].notnull() & core_df["power_num"].isnull())
    core_df["toughness_is_variable"] = (core_df["toughness"].notnull() & core_df["toughness_num"].isnull())
    core_df["loyalty_is_variable"] = (core_df["loyalty"].notnull() & core_df["loyalty_num"].isnull())
    core_df["mana_cost_is_variable"] = core_df["mana_cost"].str.contains(r"[XYZ\*]", case=False, na=False)
    core_df["pt_ratio"] = core_df["power_num"] / core_df["toughness_num"].astype("Float64")

    temp_extracted_colors = core_df["colors"].apply(lambda x: set(list(x)))
    core_df["is_colorless"] = temp_extracted_colors.apply(lambda colors: len(colors) == 0).astype("bool")
    core_df["is_white"] = temp_extracted_colors.apply(lambda colors: "W" in colors).astype("bool")
    core_df["is_blue"] = temp_extracted_colors.apply(lambda colors: "U" in colors).astype("bool")
    core_df["is_black"] = temp_extracted_colors.apply(lambda colors: "B" in colors).astype("bool")
    core_df["is_red"] = temp_extracted_colors.apply(lambda colors: "R" in colors).astype("bool")
    core_df["is_green"] = temp_extracted_colors.apply(lambda colors: "G" in colors).astype("bool")

    return core_df.sort_values(["card_id", "face_index"]).reset_index(drop=True)

def build_cards_core(cards_df: pd.DataFrame, faces_df: pd.DataFrame, types_df: pd.DataFrame, keywords_df: pd.DataFrame, sets_df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    core_df["face_count"] = faces_df.groupby("card_id")["face_index"].nunique().astype("Int64")
    core_df["text_length"] = (
    ...
    core_df["type_count"] = types_df.groupby("card_id")["type"].nunique().astype("Int64")
    core_df["keyword_count"] = keywords_df.groupby("card_id")["keyword"].nunique().astype("Int64")
    core_df["legalities_count"] = core_df["legalities"].apply(lambda x: len([v for v in x.values() if v in ("Legal", "Restricted")]) if x is not None else 0).astype("Int64")
    core_df["rulings_count"] = core_df["rulings"].apply(lambda x: len(list(x)) if x is not None else 0).astype("Int64")
    core_df["printings_count"] = core_df["printings"].apply(lambda x: len(list(x)) if x is not None else 0).astype("Int64")
    core_df["is_legal_commander"] = core_df["legalities"].apply(lambda x: x.get("commander") in ("Legal", "Restricted") if x is not None else False).astype("bool")
    core_df["is_legal_modern"] = core_df["legalities"].apply(lambda x: x.get("modern") in ("Legal", "Restricted") if x is not None else False).astype("bool")
    core_df["is_legal_pauper"] = core_df["legalities"].apply(lambda x: x.get("pauper") in ("Legal", "Restricted") if x is not None else False).astype("bool")
    core_df["is_legal_legacy"] = core_df["legalities"].apply(lambda x: x.get("legacy") in ("Legal", "Restricted") if x is not None else False).astype("bool")
